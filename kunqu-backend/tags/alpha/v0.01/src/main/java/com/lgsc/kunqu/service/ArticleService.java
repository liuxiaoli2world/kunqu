...
public class ArticleService {
	...
	public int insertArticle(Article article) {
		...
		// 插入到文章图片表
		if (article.getArticleImages() != null && article.getArticleImages().size() > 0) {
			for (ArticleImage articleImage : article.getArticleImages()) {
				articleImage.setArticleImageId(null);
				articleImage.setArticleId(article.getArticleId());
				articleImage.setCreatedAt(now);
		...
		roleTagArticle.setRoleTagId(article.getRoleTagId());
		roleTagArticle.setArticleId(article.getArticleId());
		roleTagArticle.setCreatedAt(now);
		roleTagArticleMapper.insertSelective(roleTagArticle);
	 ...
	 * @return
	 */
	public List<Article> selectByGroupId(List<Long> articleIds) {
		return  articleMapper.selectByGroupId(articleIds);
	}
