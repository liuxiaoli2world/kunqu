...
public class DramaService {
	 ...
	 * @return
	 */
	public PageInfo<Drama> selectByType(Integer pageNum, Integer pageSize, Integer type, String keyword) {
		PageInfo<Drama> pageInfo = null;
		if (type >= 0 && type <= 5) {
			PageHelper.startPage(pageNum, pageSize);
			pageInfo = new PageInfo<>(dramaMapper.selectByType(type,keyword));
		}
		return pageInfo;
	}
	...
	public int updateDrama(Drama drama) {
		...
		// 插入到剧典图片表
		if (drama.getDramaImages() != null && drama.getDramaImages().size() > 0) {
			DramaImage di = new DramaImage();
			di.setDramaId(drama.getDramaId());
			di.setImageScene("01");
			List<DramaImage> records = dramaImageMapper.select(di);
			if (records != null && records.size() > 0) {// 修改
				di.setDramaImageId(records.get(0).getDramaImageId());
				di.setImageUrl(drama.getDramaImages().get(0).getImageUrl());
				di.setUpdatedAt(now);
				dramaImageMapper.updateByPrimaryKeySelective(di);
			} else {// 插入
				DramaImage dramaImage = drama.getDramaImages().get(0);
				dramaImage.setDramaImageId(null);
				dramaImage.setDramaId(drama.getDramaId());
				dramaImage.setCreatedAt(now);
				dramaImageMapper.insertSelective(dramaImage);
